...
const genCitation = (citeproc, mode, entries, citationId, citationPre, options, isComposite) => {
  ...
  }, citationPre.length > 0 ? citationPre : [], []); // console.log(Object.getOwnPropertyNames(citeproc.registry))
  ...
  if (mode === 'note') {
    ...
    return [citationText, htmlToHast(`<span class="${((_options$inlineClass = options.inlineClass) != null ? _options$inlineClass : []).join(' ')}" id=${ids}><sup><a href="#cite-fn-${citationId}" id="cite-fnref-${citationId}" data-footnote-ref aria-describedby="footnote-label">${citationId}</a></sup></span>`)];
  } // Coerce to html to parse HTML code e.g. &#38; and return text node


  return [citationText, htmlToHast(`<span class="${((_options$inlineClass2 = options.inlineClass) != null ? _options$inlineClass2 : []).join(' ')}" id=${ids}>${citationText}</span>`)];
};
/**
  ...
  return (options = {}) => {
    return async (tree, file) => {
      ...
      visit(tree, 'text', (node, idx, parent) => {
        ...
        const [entries, isComposite] = parseCitation(match[0]); // If id is not in citation file (e.g. route alias or js package), abort process
